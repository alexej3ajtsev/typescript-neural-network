...
export class Matrix {
	...
	constructor(options?: MatrixOptions) {
		...
		let i = options.rows;
		while (i > 0) {
			const initialValue = !options.randFill
				? 0.0
	...
	setItem(rowCol: [number, number?], val: number) {
		...
		if (row + 1 > this.rows) {
			throw new Error(
				`You try set value in row #${row + 1}, but only ${this.rows} exists`
			);
		}
		if (col + 1 > this.columns) {
			throw new Error(
				`You try set value in column #${col + 1}, but only ${
					this.columns
				} exists`
			);
		}
	...
	multiply(input: Matrix | number) {
		...
		if (inputIsMatrix && this.rows !== (input as Matrix).rows) {
			throw new Error(
				"You can't multiply matricies with different rows quantity"
			);
		}
		...
		for (let i = 0; i < this.rows; i++) {
			for (let j = 0; j < this.columns; j++) {
				if (inputIsMatrix)
					result.setItem(
						[i, j],
						this.data[i][j] * (input as Matrix).data[i][j]
					);
				else result.setItem([i, j], this.data[i][j] * (input as number));
			}
		}
	...
	subtractFrom(from: Matrix | number) {
		...
		if (fromIsMatrix && this.rows !== (from as Matrix).rows) {
			throw new Error(
				"You can't subtract matrix from matrix with different rows"
			);
		}
	...
	dotProduct(matrix: Matrix) {
		if (this.columns !== matrix.rows) {
			console.log("============== DOT PRODUCT ERROR ===============");
			this.print();
			matrix.print();
			throw new Error(
				"You can't dot product this matricies, cause first.columns !== second.rows"
			);
		}
		const result = new Matrix({
			rows: this.rows,
	...
	diff(matrix: Matrix) {
		if (this.rows !== matrix.rows || this.columns !== matrix.columns) {
			throw new Error(
				"You can't get diff between matricies with different rows or columns quantities"
			);
		}
		const result = new Matrix({ rows: this.rows, columns: this.columns });
		for (let i = 0; i < this.rows; i++) {
